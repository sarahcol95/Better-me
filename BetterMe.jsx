    ...
    <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",paddingBottom:90,background:N.bg,overflow:"hidden",position:"relative"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        {tab==="oggi"&&<div>
          ...
          <RSection list={mList} type="mattina" gradStyle="linear-gradient(160deg,#FFFDF0 0%,#FFF6CC 55%,#FFFFFF 100%)" checked={checked} onToggle={k=>S.checked(p=>({...p,[k]:!p[k]}))} onAdd={setAddModal} onDel={(item,type)=>setDelModal({item,type})} onEdit={(item,newText,type)=>{if(item.key.startsWith("aw_")){S.alwaysExtra(p=>({...p,[type]:p[type].map((t,i)=>i===item.extraIdx?newText:t)}));}else if(item.key.startsWith("td_")){const idx=todayExtra[type].indexOf(item.text);S.todayExtra(p=>({...p,[type]:p[type].map((t,i)=>i===idx?newText:t)}));}}} onReorder={nl=>S.routineOrder(p=>({...p,mattina:nl.map(x=>x.key)}))} th={th} plan={plan} onFYOpen={()=>setFyOpen(true)}/>
          ...
          <RSection list={sList} type="sera" gradStyle="linear-gradient(160deg,#EEF3FF 0%,#DCE8FF 55%,#FFFFFF 100%)" checked={checked} onToggle={k=>S.checked(p=>({...p,[k]:!p[k]}))} onAdd={setAddModal} onDel={(item,type)=>setDelModal({item,type})} onEdit={(item,newText,type)=>{if(item.key.startsWith("aw_")){S.alwaysExtra(p=>({...p,[type]:p[type].map((t,i)=>i===item.extraIdx?newText:t)}));}else if(item.key.startsWith("td_")){const idx=todayExtra[type].indexOf(item.text);S.todayExtra(p=>({...p,[type]:p[type].map((t,i)=>i===idx?newText:t)}));}}} onReorder={nl=>S.routineOrder(p=>({...p,sera:nl.map(x=>x.key)}))} th={th} plan={plan} onFYOpen={()=>setFyOpen(true)}/>
        </div>}
