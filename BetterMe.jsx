...
const DD={
1:{th:TH.forza_ret,colazione:"detox",mattina:["Nettalingua + detersione viso","1 bicchiere acqua tiepida","30 Calf Raises + Vacuum 3×30″",`Supp. risveglio: ${SR}`,"Focus Glutei con pesi (40 min)","Dry Brush","Morning Skincare",`Supp. colazione: ${SC}`,"Face Yoga mattina"],alimLabel:"DETOX — zero carbo, zero sale",alimGrad:AG,alimText:AT,spuntino:"Tisana + 1 arancia",pranzo:"150–200 g proteine + doppia verdura · zero cereali · zero sale",cena:"150–200 g proteine + verdura cotta · zero carbo",sera:["Gentle Foam Roller","Dry Brush + Massaggio con crema","Skincare Sera A — Retinale 0,2%","Gambe al muro 10'","Magnesio + Potassio"],ufficio:["Oscillazioni tallone-punta","Ogni ora: 5 contrazioni glutei + Rotazione scapole","Ogni volta in bagno: 10 Squat + 20″ Shaking + 10 Wall Slides"],faceYoga:FY_M,fyLabel:"Face Yoga — Drenaggio & Scolpitura"},
...
export default function App(){
  const now=new Date(); const dow=now.getDay(); const plan=DD[dow]; const th=plan.th; const hour=now.getHours();
  ...
  const persistDaily=useCallback((key,val)=>{ storeSet("bm_d_"+key,{date:todayKey,data:val}); },[todayKey]);
  ...
  const buildList=(type)=>{
    ...
    if(order&&order.length>0){
      const keySet=new Set(raw.map(x=>x.key));
      const ordered=order.filter(k=>keySet.has(k)).map(k=>raw.find(x=>x.key===k));
      const remaining=raw.filter(x=>!order.includes(x.key));
    ...
    <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",paddingBottom:90,background:N.bg,overflow:"hidden",position:"relative"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <div style={{padding:"1.4rem 1.25rem .9rem",background:"rgba(247,245,240,0.92)",backdropFilter:"blur(20px)",borderBottom:`0.5px solid ${N.border}`}}>
          {tab==="oggi"&&<>
            <Lbl style={{marginBottom:3}}>{DN[dow].toUpperCase()} · {now.getDate()} {MN[now.getMonth()]}</Lbl>
            <Title size={28} style={{fontWeight:500,marginBottom:14,letterSpacing:-.3}}>{greeting}</Title>
        ...
        {tab==="oggi"&&<div>
          <div style={{marginBottom:12}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              ...
              <div style={{background:stepsReached?"#FFD5B8":"#fff",borderRadius:18,padding:"13px 14px",border:`0.5px solid ${stepsReached?"#E8733A":N.border}`,transition:"all .3s"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><Ic n="walk" s={18} c="#E8733A"/>{stepsReached&&<span style={{fontSize:10,padding:"2px 8px",borderRadius:99,background:"#A04010",color:"#fff",fontFamily:F.body,fontWeight:600}}>✓</span>}</div>
                <Lbl>PASSI</Lbl>
          ...
          <div style={{borderRadius:20,padding:"15px 16px",marginBottom:12,background:plan.alimGrad,border:`0.5px solid ${N.border}`}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}><div style={{display:"flex",alignItems:"center",gap:7}}><Ic n="fork" s={15} c={plan.alimText}/><Title size={18} style={{color:plan.alimText}}>Alimentazione</Title></div><button onClick={()=>{setTab("altro");setSubTab("proteine");}} style={{display:"flex",alignItems:"center",gap:4,fontSize:10,padding:"4px 10px",borderRadius:20,border:`1px solid ${plan.alimText}44`,background:"rgba(255,255,255,0.55)",color:plan.alimText,cursor:"pointer",fontFamily:F.body,fontWeight:600}}><Ic n="protein" s={11} c={plan.alimText}/>Proteine ›</button></div>
            <div style={{background:"rgba(255,255,255,.6)",borderRadius:13,padding:"10px 12px",marginBottom:8}}><Lbl>COLAZIONE</Lbl><p style={{fontSize:12,color:N.text,margin:0,lineHeight:1.55,fontFamily:F.body}}>{plan.colazione==="detox"?COLAZIONE.detox:COLAZIONE.std}</p></div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:10}}>
              {[["Spuntino",plan.spuntino],["Pranzo",plan.pranzo],["Cena",plan.cena]].map(([lbl,val])=>(<div key={lbl} style={{background:"rgba(255,255,255,.6)",borderRadius:13,padding:"9px 10px"}}><Lbl>{lbl.toUpperCase()}</Lbl><p style={{fontSize:11,color:N.text,margin:0,lineHeight:1.45,fontFamily:F.body}}>{val}</p></div>))}
            </div>
          ...
          <div style={{borderRadius:20,padding:"14px 15px",marginBottom:12,background:"linear-gradient(135deg,#F7F5F0,#EDE9E2)",border:`0.5px solid ${N.border}`}}>
        ...
        {tab==="altro"&&subTab==="ricette"&&<div>
          ...
          <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:8,marginBottom:12,scrollbarWidth:"none"}}>
            {["Tutti","Ricette","Idee Pranzo/Cena","Carne","Pesce","Uova","Tofu","Legumi","Primi","Vegan","Detox","Leggero","Anti-infiamm.","Post-workout"].map(f=><button key={f} onClick={()=>setRicFilter(f)} style={{flexShrink:0,fontSize:11,padding:"5px 12px",borderRadius:20,border:`1px solid ${ricFilter===f?N.text:N.border}`,background:ricFilter===f?N.text:"#fff",color:ricFilter===f?"#fff":N.muted,cursor:"pointer",fontFamily:F.body,fontWeight:ricFilter===f?600:400,whiteSpace:"nowrap"}}>{f}</button>)}
          </div>
